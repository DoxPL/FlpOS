...
static void builtin_cmp(const char *[], uint8_t);
...
static void builtin_cmp(const char *args[], uint8_t argc) {
    int32_t lvalue, rvalue;
    char operand;
    if (argc >= 3) {
        lvalue = atoi((char*)args[0]);
        operand = *args[1];
        rvalue = atoi((char*)args[2]);
        switch (operand) {
            case '>':
                last_rval = lvalue > rvalue;
                break;
            case '<':
                last_rval = lvalue < rvalue;
                break;
